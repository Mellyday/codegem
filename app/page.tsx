    ...
    <main className="min-h-screen bg-gradient-to-b from-cyan-50 via-teal-50/80 to-emerald-50/60 text-slate-900">
      <section className="mx-auto flex max-w-3xl flex-col gap-8 px-6 py-16">
        <header className="space-y-2">
          <h1 className="text-3xl font-semibold text-cyan-700">Your Code</h1>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          <section className="space-y-3">
            <h2 className="text-xl font-semibold text-cyan-700">Repos</h2>
            {repos.length === 0 ? (
              <p className="italic text-cyan-400">No repos yet.</p>
            ) : (
          ...
          <section className="space-y-3">
            <h2 className="text-xl font-semibold text-cyan-700">Projects</h2>
            {projects.length === 0 ? (
              <p className="italic text-cyan-400">No projects yet.</p>
            ) : (
              <ul className="grid gap-2">
                {projects.map((p) => (
                  <RepoProjectItem
